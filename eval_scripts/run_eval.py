...
if not os.path.exists(f"/data/manifolds/pybullet_eval/{args.exp_name}"):
    os.mkdir(f"/data/manifolds/pybullet_eval/{args.exp_name}")
...
for var_cfg in exp_cfg.variants:
    ...
    # only run up to max trials accounting for existing trials
    trials = os.listdir(f"/data/manifolds/pybullet_eval/{args.exp_name}")
...
#         --ckpt='/data/manifolds/fb_runs/multirun/pq-pq_mini_relabel/2022-05-27_161440/0/default_default/15_15/checkpoints/epoch=675-step=909345.ckpt' \
#         --use_min_cost_traj=1 \
#         -o /data/manifolds/pybullet_eval/${EXP_NAME}_mincosttraj
# done
