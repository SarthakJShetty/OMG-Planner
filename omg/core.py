...
class Env(object):
    ...
    def combine_sdfs(self):
        ...
        if num > 0:
            max_shape = np.array([obj.sdf.data.shape for obj in self.objects]).max(
                axis=0
            )
        else:
...
class PlanningScene(object):
    ...
    def __init__(self, cfg):
        ...
        self.traj = Trajectory(
            timesteps=cfg.timesteps, start_end_equal=cfg.start_end_equal
        )
    ...
    def reset_env(self, joints=None):
        ...
        if joints is not None:
            ...
            self.traj = Trajectory(
                self.cfg.timesteps, start_end_equal=self.cfg.start_end_equal
            )
    ...
    def fast_debug_vis(
        ...
        def fast_vis_collision(poses, cls_indexes, i, collision_pts, interact):
            ...
            vis_pt_with_grad = (
                -collision_pts[i - 1, :, :, 9:12].reshape(-1, 3).T * 0.02 + vis_pt
            )  #
            return self.renderer.vis(
                poses,
                cls_indexes,
                interact=interact,
                visualize_context={
                    "line": [(vis_pt_with_grad, vis_pt)],
                    "project_point": [vis_pt],
        ...
        def fast_vis_goalset(poses, cls_indexes, i, traj, interact, traj_idx=None):
            ...
            if vis_goal_set:
                ...
                mix_frame_image[bg_mask] = (
                    bg_image[bg_mask] * 0.5 + mix_frame_image[bg_mask] * 0.5
                )
        ...
        def fast_debug_traj(traj, traj_idx=None):
            ...
            for i in range(traj.shape[0]):
                ...
                if collision_pt and i > 0 and i < self.cfg.timesteps:
                    frames.append(
                        fast_vis_collision(
                            poses, cls_indexes, i, collision_pts, interact
                        )
                    )
                elif goal_set and i > 0:
                    frame, mask = fast_vis_goalset(
                        poses, cls_indexes, i, traj, interact, traj_idx=traj_idx
                    )
                    frames.append(frame)
                    masks.append(mask)
                else:
                    frame, mask = fast_vis_simple(
                        poses, cls_indexes, interact, traj_idx=traj_idx
                    )
                    frames.append(frame)
                    masks.append(mask)
    ...
    def setup_renderer(self):
        ...
        models = [self.cfg.robot_model_path + "/{}.DAE".format(item) for item in links]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--file", help="filename", type=str, default="demo_scene_1"
    )
    parser.add_argument("-w", "--write_video", help="write video", action="store_true")
    parser.add_argument(
        "-g", "--grasp", help="grasp initialization", type=str, default="grasp"
    )
    parser.add_argument(
        "-exp", "--experiment", help="loop through the 100 scenes", action="store_true"
    )
    ...
    if not args.experiment:
        ...
        if args.vis or args.write_video:
            scene.fast_debug_vis(
                interact=int(args.vis),
                write_video=args.write_video,
                nonstop=True,
                collision_pt=args.vis_collision_pt,
                goal_set=args.vis_goalset,
            )
    else:
