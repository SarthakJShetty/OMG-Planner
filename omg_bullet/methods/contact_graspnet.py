...
class ContactGraspNetInference:
    def __init__(self, save_results=False, visualize=False):
        ...
        self.global_config = config_utils.load_config(
            self.args.ckpt_dir,
            batch_size=self.args.forward_passes,
            arg_configs=self.args.arg_configs,
        )
            ...
            if False:
                ...
                grasps = [
                    create_gripper_marker(
                        color=[0, 255, 0], tube_radius=0.001
                    ).apply_transform(T)
                    for T in pred_grasps_cam[-1]
                ]
    ...
    def get_args(self):
        ...
        parser.add_argument(
            "--ckpt_dir",
            default=f"{os.path.dirname(contact_graspnet.__file__)}/../checkpoints/scene_test_2048_bs3_hor_sigma_001",
