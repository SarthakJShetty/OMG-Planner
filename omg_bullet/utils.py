...
def add_line(
    start, end, color=BLACK, width=1, lifetime=0, parent=NULL_ID, parent_link=BASE_LINK
):
...
def draw_pose(T, length=0.1, d=3, alt_color=False, **kwargs):
    origin_world = T @ np.array((0.0, 0.0, 0.0, 1.0))
    dbg_ids = []
    for k in range(d):
...
def get_world2cam_transform(env):
    T_world2camgl = np.linalg.inv(
        np.asarray(env._view_matrix).reshape((4, 4), order="F")
    )
...
def bullet_execute_plan(env, plan, write_video, video_writer):
    ...
    if write_video:
        for robs in ret_obs:
            ...
            video_writer.write(
                robs["rgb"][0][:, :, [2, 1, 0]].astype(np.uint8)
            )  # to get enough frames to save
    return rew
...
def objinfo_from_obj(grasp_h5s, mesh_root, objname):
    ...
    objhash = os.listdir(f"{mesh_root}/meshes/{objname}")[0].replace(
        ".obj", ""
    )  # [HASH]
    grasp_prefix = f"{objname}_{objhash}"  # Book_[HASH]
    for (
        grasp_h5
    ) in grasp_h5s:  # Get file in grasps/ corresponding to this book, hash, and scale
    ...
    obj_mesh, T_ctr2obj = load_mesh(
        f"{mesh_root}/grasps/{graspfile}", mesh_root_dir=mesh_root, load_for_bullet=True
    )
